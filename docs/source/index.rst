...
a technique that could be implemented with only code in Python's standard
library.
...
opinion, and merely provides some optional utilities to make it easier to
work in the DI style. For this reason, most of this manual describes a way
to do DI *without* this library, only at the end introducing the motivation
for ``tiedye`` and its usage.
